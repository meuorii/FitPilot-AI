...
export function WorkoutsPage() {
  ...
  const [
    ...
    useState<WorkoutRoutineExercise | null>(
      null,
    )
   ...
   * It acts as a fallback whenever a nested routine exercise
   * only contains exercise_id but not the full exercise object.
   */
  const exercises =
  ...
  const handleActivateSplit =
    async (
      ...
      try {
        const response =
          await activateSplitMutation.mutateAsync(
            splitId,
          )
        ...
        showToast({
          ...
          subheading:
            response.message ||
            `${response.data.name} is now active.`,
        })
      } catch (error) {
        showToast({
          type: 'error',
          heading:
            'Could not change split',
          subheading:
            error instanceof Error
              ? error.message
              : 'Please try again.',
  ...
  const handleCreateRoutine =
    async (
      input: CreateWorkoutRoutineInput,
    ) => {
      try {
        const response =
        ...
        showToast({
          ...
          subheading:
            response.message ||
            `${response.data.name} was created.`,
        })
