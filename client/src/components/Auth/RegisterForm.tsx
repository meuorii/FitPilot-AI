...
function FieldLabel({ icon, children, iconClassName = 'size-[19px]' }: FieldLabelProps) {
  return (
    <span className="flex items-center gap-2 text-[17px] font-normal leading-none text-[#38323F]">
      <img src={icon} alt="" aria-hidden="true" draggable={false} className={`shrink-0 select-none object-contain transition-transform duration-300 group-focus-within:-translate-y-0.5 ${iconClassName}`} />
      {children}
    </span>
  )
}
...
function PasswordToggle({ visible, onToggle, label }: PasswordToggleProps) {
  return (
    <button type="button" onClick={onToggle} className="absolute -right-1 top-1/2 grid size-8 -translate-y-1/2 place-items-center rounded-full transition-colors hover:bg-[#7482A4]/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#7482A4]/35 active:scale-95" aria-label={`${visible ? 'Hide' : 'Show'} ${label}`} aria-pressed={visible}>
      <img key={visible ? `hide-${label}` : `show-${label}`} src={visible ? ICONS.hidePassword : ICONS.viewPassword} alt="" aria-hidden="true" draggable={false} className="register-password-icon size-4 select-none object-contain" />
    </button>
  )
}
    ...
    <section className="w-full max-w-[435px]" aria-labelledby="register-title">
      ...
      <form onSubmit={handleSubmit} className="w-full">
        <div className="space-y-[clamp(28px,4.1vh,42px)]">
          ...
          <label className="register-stagger-item register-stagger-3 group block" htmlFor="register-email">
            <FieldLabel icon={ICONS.email} iconClassName="size-[18px]">Email</FieldLabel>
            <input id="register-email" name="email" type="email" autoComplete="email" value={formData.email} onChange={(event) => setFormData((previous) => ({ ...previous, email: event.target.value }))} placeholder="example@gmail.com" required className="mt-[13px] h-[31px] w-full rounded-none border-0 border-b border-[#38323F]/80 bg-transparent px-0 pb-[7px] text-[15px] leading-5 text-[#38323F] outline-none transition-colors duration-200 placeholder:text-[#38323F]/25 focus:border-[#7482A4] focus:ring-0" />
          </label>
          <div className="register-stagger-item register-stagger-4 group block">
