          ...
          <form className="relative z-10 flex min-h-[520px] w-full flex-col items-center px-6 pb-10 pt-[68px] md:absolute md:left-[23px] md:top-[62px] md:h-[444px] md:min-h-0 md:w-[483px] md:px-0 md:py-0" onSubmit={handleSubmit}>
            ...
            <div aria-label="Six-digit verification code" className="email-verification-code mt-[42px] flex w-full max-w-[394px] justify-between gap-2 md:absolute md:left-[45px] md:top-[180px] md:mt-0" onPaste={(event) => { event.preventDefault(); distributeDigits(event.clipboardData.getData('text')) }}>
              {code.map((digit, index) => (
                <input aria-label={`Verification code digit ${index + 1}`} autoComplete={index === 0 ? 'one-time-code' : 'off'} className="email-verification-digit h-[46px] min-w-0 flex-1 rounded-t-[9px] border-0 border-b-[1.5px] border-[#8E8993] bg-white/45 p-0 text-center text-[22px] font-semibold leading-none text-[#38323F] caret-[#7482A4] outline-none transition-[border-color,background-color,box-shadow,transform] duration-200 hover:bg-white/70 focus:-translate-y-0.5 focus:border-[#7482A4] focus:bg-white/85 focus:shadow-[0_8px_18px_rgba(116,130,164,0.14)]" inputMode="numeric" key={index} maxLength={1} onChange={(event) => handleChange(index, event.target.value)} onFocus={(event) => event.currentTarget.select()} onKeyDown={(event) => handleInputKeyDown(index, event)} pattern="[0-9]*" ref={(element) => { inputRefs.current[index] = element }} style={{ animationDelay: `${245 + index * 45}ms` }} type="text" value={digit} />
              ))}
            </div>
