...
import type { WorkoutSessionDetails } from '../../services/types/workout'
...
export function QuickSessionPreview({
  ...
  const mutation = useLogWorkoutSet(session?.id ?? null)
  const current = session?.progress.current_exercise ?? null
  const [weight, setWeight] = useState('')
  const [reps, setReps] = useState('')
  const [restSeconds, setRestSeconds] = useState(0)

  const lastMatchingSet = useMemo(() => {
  ...
  useEffect(() => {
    ...
    setWeight(lastMatchingSet ? String(lastMatchingSet.weight_kg) : '')
    setReps(String(current.target_reps_min))
  }, [current?.exercise_id, lastMatchingSet?.id])
      ...
      <section className="rounded-[24px] border border-[#EAE7EC] bg-white p-5 shadow-[0_8px_28px_rgba(56,50,63,0.045)] sm:p-6">
        ...
        <div className="mt-5 rounded-2xl bg-[#F7F6F8] px-5 py-8 text-center">
          ...
          <p className="mx-auto mt-1 max-w-sm text-xs leading-5 text-[#8B8690]">
            Start today&apos;s routine to log sets, track rest, and watch your session progress update live.
          </p>
        </div>
      </section>
    )
  ...
  const canComplete =
    ...
    Number.isFinite(validWeight) &&
    validWeight >= 0 &&
    Number.isFinite(validReps) &&
    validReps > 0

  const handleCompleteSet = async () => {
    if (!current || !nextSetNumber || !canComplete) return
    ...
    try {
      ...
      if (response.data.rest.should_start) {
        setRestSeconds(response.data.rest.rest_seconds)
      } else {
        setRestSeconds(0)
      }
      ...
      showToast({
        ...
        subheading: response.message || 'Your workout progress has been saved.',
      })
    } catch (error) {
      showToast({
        type: 'error',
    ...
    <section className="rounded-[24px] border border-[#EAE7EC] bg-white p-5 shadow-[0_8px_28px_rgba(56,50,63,0.045)] sm:p-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-[#8B8690]">Live workout tracking</p>
        </div>
        <button
          type="button"
      ...
      <div className="mt-5 rounded-2xl bg-[#F9F9FC] p-4">
        ...
        <p className="mt-1 line-clamp-2 text-base font-extrabold text-[#38323F]">
          {current?.exercise?.name ?? 'Workout complete'}
        </p>

        {current ? (
          <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-5">
            <div className="rounded-xl border border-[#EAE7EC] bg-white p-3">
              <p className="text-[10px] font-semibold text-[#8B8690]">Set Progress</p>
              <p className="mt-1 text-sm font-extrabold text-[#38323F]">
                {nextSetNumber ?? current.target_sets} of {current.target_sets}
              </p>
            </div>
            <div className="rounded-xl border border-[#EAE7EC] bg-white p-3">
              <p className="text-[10px] font-semibold text-[#8B8690]">Target Reps</p>
              <p className="mt-1 text-sm font-extrabold text-[#38323F]">
                {current.target_reps_min}–{current.target_reps_max}
              </p>
            </div>
            <label className="rounded-xl border border-[#EAE7EC] bg-white p-3">
              <span className="block text-[10px] font-semibold text-[#8B8690]">Weight kg</span>
              <input
                type="number"
                min="0"
                step="0.5"
            ...
            <label className="rounded-xl border border-[#EAE7EC] bg-white p-3">
              <span className="block text-[10px] font-semibold text-[#8B8690]">Reps</span>
              <input
                type="number"
                min="1"
                value={reps}
            ...
            <div className="rounded-xl border border-[#EAE7EC] bg-white p-3">
              <p className="text-[10px] font-semibold text-[#8B8690]">Rest Timer</p>
      ...
      {current ? (
        <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3">
          ...
          >
            ...
            {mutation.isPending ? 'Saving...' : 'Complete Set'}
          </button>
          <button
            type="button"
      ...
      <div className="mt-5">
        <div className="flex items-center justify-between gap-4">
          <p className="text-xs font-bold text-[#5F5963]">Session Progress</p>
          <p className="text-xs font-extrabold text-[#7482A4]">{Math.round(percentage)}%</p>
        </div>
        ...
        <div className="mt-4 grid grid-cols-3 divide-x divide-[#EAE7EC] text-center">
          <div>
            <p className="text-sm font-extrabold text-[#38323F]">
              {progress.completed_exercises} / {progress.planned_exercises}
            </p>
            <p className="mt-0.5 text-[10px] text-[#8B8690]">exercises</p>
          </div>
          <div>
            <p className="text-sm font-extrabold text-[#38323F]">
              {progress.completed_sets} / {progress.planned_sets}
            </p>
            <p className="mt-0.5 text-[10px] text-[#8B8690]">sets</p>
          </div>
          <div>
            <p className="text-sm font-extrabold text-[#38323F]">
              {session.total_volume_kg.toLocaleString()} kg
            </p>
            <p className="mt-0.5 text-[10px] text-[#8B8690]">volume</p>
          </div>
        </div>
      </div>
    </section>
