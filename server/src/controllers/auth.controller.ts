...
const verificationCodesMatch = (
  ...
  const submittedHashBuffer = Buffer.from(
    hashVerificationCode(submittedCode),
    'hex',
  );
...
export const register = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { data: newUser, error: insertError } = await supabaseAdmin
      ...
      .select('id, email, full_name, is_onboarded, email_verified')
      .single();
    ...
    try {
      ...
      console.error('Register email delivery error:', emailError);
      ...
      res.status(503).json({
        ...
        message:
          'Your account could not be completed because the verification email service is temporarily unavailable. Please try again.',
      });
      return;
    }
...
export const verifyEmail = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { data: user, error } = await supabaseAdmin
      .from('profiles')
      .select(
        'id, email, full_name, is_onboarded, email_verified, email_verification_code, email_verification_expires_at',
      )
      .eq('email', normalizedEmail)
      .maybeSingle();
    ...
    if (
      ...
      res.status(400).json({
        ...
        code: 'NO_ACTIVE_VERIFICATION_CODE',
      });
      return;
    }
    ...
    res.status(200).json({
      ...
      data: {
        ...
        user: {
          ...
          is_onboarded: verifiedUser.is_onboarded,
          email_verified: verifiedUser.email_verified,
        },
        next_step: verifiedUser.is_onboarded ? 'dashboard' : 'onboarding',
      },
    });
  } catch (err) {
...
export const resendVerificationCode = async (
  ...
  try {
    ...
    const previousCode = user.email_verification_code;
    const previousExpiresAt = user.email_verification_expires_at;
    const verificationCode = randomInt(100000, 1000000).toString();
    const hashedVerificationCode = hashVerificationCode(verificationCode);
    const verificationExpiresAt = new Date(
      Date.now() + VERIFICATION_CODE_TTL_MS,
    ...
    try {
      ...
      if (rollbackError) {
        console.error('Resend rollback error:', rollbackError);
      }
      ...
      res.status(503).json({
        ...
        code: 'EMAIL_SERVICE_UNAVAILABLE',
        message:
          'The verification email could not be sent right now. Please try again.',
      });
      return;
    }
...
export const login = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { data: user, error } = await supabaseAdmin
      .from('profiles')
      .select(
        'id, email, full_name, password_hash, is_onboarded, email_verified',
      )
      .eq('email', normalizedEmail)
      .maybeSingle();
