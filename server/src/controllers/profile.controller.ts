...
export const updateMyProfile = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = req.user?.id;
    if (!userId) { res.status(401).json({ success: false, error: 'Unauthorized', message: 'User ID missing from request' }); return; }
    const allowedFields = ['full_name', 'avatar_url', 'age', 'gender', 'height_cm', 'current_weight_kg', 'activity_level', 'fitness_experience', 'unit_system', 'theme', 'auto_recalculate_goals'];
    ...
    if (currentProfile) {
      const mergedStats = { age: updates.age !== undefined ? Number(updates.age) : currentProfile.age, gender: updates.gender || currentProfile.gender, height_cm: updates.height_cm !== undefined ? Number(updates.height_cm) : Number(currentProfile.height_cm), weight_kg: updates.current_weight_kg !== undefined ? Number(updates.current_weight_kg) : Number(currentProfile.current_weight_kg), activity_level: updates.activity_level || currentProfile.activity_level, primary_goal: currentProfile.primary_goal };
