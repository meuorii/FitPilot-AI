...
export const getRoutines = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { data, error } = await supabaseAdmin.from('workout_routines').select(`id, user_id, name, description, cover_image_url, is_global, created_at, updated_at, routine_exercises (id, target_sets, target_reps_min, target_reps_max, order_index, exercises (id, name, category, equipment, difficulty, image_url))`).or(filter).order('created_at', { ascending: false });
...
export const getExercises = async (_req: Request, res: Response): Promise<void> => {
  try {
    const { data, error } = await supabaseAdmin.from('exercises').select('id, name, category, equipment, difficulty, instructions, image_url, created_at').order('name', { ascending: true });
    if (error) throw error;
    res.status(200).json({ success: true, data: data ?? [] });
  } catch (error) {
...
export const createCustomRoutine = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { data: routine, error: routineError } = await supabaseAdmin.from('workout_routines').insert({ user_id: userId, name: String(name).trim(), description: description || null, cover_image_url: cover_image_url || null, is_global: false }).select().single();
    ...
    if (exercises && exercises.length > 0) {
      ...
      const { error: exercisesError } = await supabaseAdmin.from('routine_exercises').insert(routineExercisesData);
      if (exercisesError) {
    ...
    const { data: completeRoutine, error: fetchError } = await supabaseAdmin.from('workout_routines').select(`id, user_id, name, description, cover_image_url, is_global, created_at, updated_at, routine_exercises (id, target_sets, target_reps_min, target_reps_max, order_index, exercises (id, name, category, equipment, difficulty, image_url))`).eq('id', routine.id).single();
...
export const logWorkoutSession = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { data: session, error: sessionError } = await supabaseAdmin.from('workout_sessions').insert({ user_id: userId, routine_id: routine_id || null, workout_date: workout_date || new Date().toISOString().split('T')[0], notes: notes || null, total_volume_kg: totalVolume }).select().single();
    ...
    const { error: setsError } = await supabaseAdmin.from('workout_sets').insert(setsPayload);
    if (setsError) {
      await supabaseAdmin.from('workout_sessions').delete().eq('id', session.id).eq('user_id', userId);
      throw setsError;
    }
